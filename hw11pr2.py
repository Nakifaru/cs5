class Board:
    ...
    def colsToWin(self, ox):
        """Returns the columns that the argument O or X can win in the next move
        """
        wins = []
    ...
    def aiMove(self, ox):
        """Returns the best move to be played by the ai, to win or if it can't win, to block, 
           of there is nothing to block, it will play a random move.
        """
        ...
        win = self.colsToWin(ox)
        lose = self.colsToWin(opp)
        ...
        if win:
            ...
            centre = (self.width // 2)
            if self.data[self.height - 1][centre] == ' ':
                return centre
            if self.data[self.height - 1][0] == ' ':
                return 0
            if self.data[self.height - 1][self.width - 1] == ' ':
                return self.width - 1
            
            inc = 0
            for x in range(0, centre - 1):
                play1 = centre + inc
                play2 = centre - inc
                if self.allowsMove(play1):
                    return play1
                elif self.allowsMove(play2):
                    return play2
                inc += 1
